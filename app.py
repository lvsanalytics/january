...
def store_and_retrieve_conversation(user_input, bot_response, user_name):
    ...
    # Add the user input and bot response embeddings to ChromaDB, only storing user_name
    collection.add(
        ids=[user_input_id, bot_response_id],
        embeddings=[user_input_embedding, bot_response_embedding],
        documents=[user_input, bot_response],
        metadatas=[{"user_name": user_name}, {"user_name": user_name}]
    )
...
# Helper function to extract URLs from a message
def extract_urls_from_text(text):
    url_pattern = re.compile(r'(https?://\S+)')
    urls = url_pattern.findall(text)
    return urls

# Helper function to download file from Slack
